...
- The `HasFields` trait is now used for the common `#[derive(Fields)]` and provides `field_names() -> &'static [&'static str]` and `field_metas()` to access field metadata.
...
|                                                       | `field::HasFields::field_metas()` with `FieldMeta` struct             |
